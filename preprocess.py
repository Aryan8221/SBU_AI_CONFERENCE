...
def preprocess_nifti_images(in_dir, out_dir):
    ...
    for ct_filename, pet_filename in tqdm(zip(ct_files, pet_files)):
        ...
        # pet_data_norm = (clipped_pet_data - np.min(clipped_pet_data)) / (
        ...
        if clipped_pet_data.shape == clipped_pet_data.shape:
            ct_output_path = os.path.join(out_dir, "CT", os.path.splitext(os.path.basename(ct_filename))[0] + ".nii.gz")
            pet_output_path = os.path.join(out_dir, "PT",
                                           os.path.splitext(os.path.basename(pet_filename))[0] + ".nii.gz")
